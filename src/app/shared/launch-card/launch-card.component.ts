...
@Component({
  ...
  template: `
    <aside *ngIf="launch">
      <h3>
        <a id="launch-name" [routerLink]="['/launch/' + launch.slug]">{{ launch.name }}</a>
      </h3>
      ...
      <button data-testid="add-fav-button" *ngIf="allowAddToFavorites" (click)="onClicAddToFavorites()">
        Add to favorites 🤍
      </button>
      <button data-testid="remove-fav-button" *ngIf="allowRemoveFromFavorites" (click)="onClicRemoveFromFavorites()">
